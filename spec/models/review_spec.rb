...
RSpec.describe Review, type: :model do
  ...
  describe 'test validator' do
    ...
    it 'Checks validator errors message' do
      expect(review1).to be_valid
      expect(review2).to_not be_valid
      expect(review2.errors.full_messages).to eq(['User did not book this item'])

      expect(review3).to_not be_valid
      expect(review3.errors.full_messages).to eq(['User did not book any item in a reviewable user'])
    end
  end
  describe '.from_item_by_user_id' do
    ...
    it 'returns reviews for all item by user id' do
